...
async fn test_clone_and_apply_basic_repo() -> Result<()> {
    ...
    // The file should be created in the real home directory under the test path
    let target_file = env.test_path("test-app/config.txt");
...
async fn test_multiple_subdirectories() -> Result<()> {
    ...
    let file1 = env.test_path("multi-app1/config.txt");
    let file2 = env.test_path("multi-app2/config.txt");
