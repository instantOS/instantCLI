...
impl RenderPipeline {
    ...
    fn build_filter_complex(&self, bindings: &[TimelineBinding]) -> String {
        // Use time-based overlay approach for better performance
        self.build_time_based_overlay_filter(bindings)
    }

    /// Simple filter complex with proper split handling
    fn build_simple_filter_complex(&self, bindings: &[TimelineBinding]) -> String {
        let mut filters: Vec<String> = Vec::new();
        let mut concat_inputs = String::new();

        // Step 1: Count overlay usage and create split filters
        let mut overlay_usage = std::collections::HashMap::new();
        for binding in bindings.iter() {
            if let TimelineBinding::Clip { overlay_input: Some(overlay_index) } = binding {
                *overlay_usage.entry(*overlay_index).or_insert(0) += 1;
            }
        }

        // Step 2: Create split filters for ALL overlays (even single use)
        let mut overlay_outputs = std::collections::HashMap::new();
        for (&overlay_index, &count) in overlay_usage.iter() {
            // Process the overlay once
            let processed_label = format!("overlay_proc_{}", overlay_index);
            filters.push(format!(
                "[{input}:v]scale=w=ceil({width}*0.6/2)*2:h=-1:flags=lanczos,setsar=1,format=rgba,colorchannelmixer=aa=0.85[{processed}]",
                input = overlay_index,
                width = self.target_width,
                processed = processed_label,
            ));

            // Always split, even for single use (fixes timestamp sync issues)
            let split_labels: Vec<String> = (0..count)
                .map(|i| format!("overlay_{}_{}", overlay_index, i))
                .collect();
            ...
            overlay_outputs.insert(overlay_index, split_labels);
        }

        // Step 3: Build main filter chain
        let mut overlay_counters = std::collections::HashMap::new();
            ...
            match (item, binding) {
                (TimelineItem::Clip(segment), TimelineBinding::Clip { overlay_input }) => {
                    ...
                    let final_label = if let Some(overlay_index) = overlay_input {
                        ...
                        // Always use split output (all overlays are now split)
                        let counter = overlay_counters.entry(*overlay_index).or_insert(0);
                        let overlay_label = overlay_outputs[overlay_index][*counter].clone();
                        *counter += 1;
    ...
    /// Time-based overlay approach - concatenate first, then apply overlays with time conditions
    fn build_time_based_overlay_filter(&self, bindings: &[TimelineBinding]) -> String {
        let mut filters: Vec<String> = Vec::new();
        let mut concat_inputs = String::new();

        // Step 1: Build concatenation of all video/audio segments
        for (idx, (item, binding)) in self.timeline.iter().zip(bindings.iter()).enumerate() {
            let audio_label = format!("a{idx}");

            match (item, binding) {
                (TimelineItem::Clip(segment), TimelineBinding::Clip { overlay_input: _ }) => {
                    let video_label = format!("v{idx}");

                    // Trim video and audio segments (no overlay processing here)
                    filters.push(format!(
                        "[0:v]trim=start={start}:end={end},setpts=PTS-STARTPTS[{video}]",
                        start = format_time(segment.start),
                        end = format_time(segment.end),
                        video = video_label,
                    ));
                ...
                (TimelineItem::TitleCard(_title_card), TimelineBinding::TitleCard { input_index }) => {
                    let video_label = format!("v{idx}");
                ...
                _ => {
                    // Mismatched item and binding types - this shouldn't happen in normal operation
                    panic!("Mismatched timeline item and binding types at index {}", idx);
                }
            }
        }
        ...
        // Step 3: Collect overlay time ranges and apply overlays
        let overlay_ranges = self.collect_overlay_time_ranges(bindings);

        if overlay_ranges.is_empty() {
            // No overlays, just use concatenated video
            filters.push("[concat_v]copy[outv]".to_string());
        } else {
            // Apply overlays with time-based enabling
            self.apply_time_based_overlays(&mut filters, &overlay_ranges);
        }
    ...
    /// Test method: Build filter for single overlay test
    #[allow(dead_code)]
    fn build_test_single_overlay(&self, video_start: f64, video_end: f64, overlay_input: usize) -> String {
        format!(
            "[0:v]trim=start={start}:end={end},setpts=PTS-STARTPTS[base]; \
             [0:a]atrim=start={start}:end={end},asetpts=PTS-STARTPTS[audio]; \
             [{overlay}:v]scale=w=ceil({width}*0.6/2)*2:h=-1:flags=lanczos,setsar=1,format=rgba,colorchannelmixer=aa=0.85[ov]; \
             [base][ov]overlay=x=(W-w)/2:y=(H-h)/2[outv]",
            start = format_time(video_start),
            end = format_time(video_end),
            overlay = overlay_input,
            width = self.target_width,
        )
    }

    /// Test method: Build filter for concatenation without overlays
    #[allow(dead_code)]
    fn build_test_concat_only(&self, segments: &[(f64, f64)]) -> String {
        ...
        filters.push(format!(
            "{inputs}concat=n={count}:v=1:a=1[outv][outa]",
            inputs = concat_inputs,
            count = segments.len()
        ));
    ...
    /// Collect overlay time ranges from timeline bindings
    fn collect_overlay_time_ranges(&self, bindings: &[TimelineBinding]) -> Vec<(usize, Vec<(f64, f64)>)> {
        let mut overlay_ranges: std::collections::HashMap<usize, Vec<(f64, f64)>> = std::collections::HashMap::new();
        let mut current_time = 0.0;

        // Track consecutive segments with the same overlay
        let mut current_overlay: Option<usize> = None;
        let mut current_overlay_start = 0.0;

        for (item, binding) in self.timeline.iter().zip(bindings.iter()) {
            match (item, binding) {
                (TimelineItem::Clip(segment), TimelineBinding::Clip { overlay_input }) => {
                    let duration = segment.end - segment.start;

                    // Check if overlay is changing
                    if current_overlay != *overlay_input {
                        // End current overlay if one is active
                        if let Some(overlay_index) = current_overlay {
                            overlay_ranges
                                .entry(overlay_index)
                                .or_insert_with(Vec::new)
                                .push((current_overlay_start, current_time));
                        }

                        // Start new overlay if present
                        current_overlay = *overlay_input;
                        if overlay_input.is_some() {
                            current_overlay_start = current_time;
                        }
                    }

                    current_time += duration;
                }
                (TimelineItem::TitleCard(_), TimelineBinding::TitleCard { input_index: _ }) => {
                    // Title cards end any active overlay
                    if let Some(overlay_index) = current_overlay {
                        overlay_ranges
                            .entry(overlay_index)
                            .or_insert_with(Vec::new)
                    ...
                    current_time += 2.0; // or 5.0 for pause cards
                }
                _ => {
                    // Mismatched item and binding types - this shouldn't happen in normal operation
                    panic!("Mismatched timeline item and binding types in overlay collection");
                }
            }
        }

        // End any remaining active overlay at the end of the timeline
        if let Some(overlay_index) = current_overlay {
            overlay_ranges
                .entry(overlay_index)
                .or_insert_with(Vec::new)
        ...
        overlay_ranges.into_iter().collect()
    }

    /// Apply time-based overlays to the concatenated video
    fn apply_time_based_overlays(&self, filters: &mut Vec<String>, overlay_ranges: &[(usize, Vec<(f64, f64)>)]) {
        let mut current_video_label = "concat_v".to_string();

        for (overlay_index, ranges) in overlay_ranges {
            let overlay_label = format!("overlay_{}", overlay_index);
            let output_label = format!("overlaid_{}", overlay_index);

            // Process the overlay image
            filters.push(format!(
                "[{input}:v]scale=w=ceil({width}*0.6/2)*2:h=-1:flags=lanczos,setsar=1,format=rgba,colorchannelmixer=aa=0.85[{overlay}]",
                input = overlay_index,
                width = self.target_width,
                overlay = overlay_label,
            ));

            // Build time condition for when overlay should be visible
            let time_conditions: Vec<String> = ranges
                .iter()
                .map(|(start, end)| format!("between(t,{},{})", start, end))
                .collect();
            let enable_condition = time_conditions.join("+");
