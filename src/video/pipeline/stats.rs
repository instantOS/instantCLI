...
pub async fn handle_stats(args: StatsArgs) -> Result<()> {
    let report = build_stats_report(args).await?;
    emit_report(&report);
    Ok(())
}

pub async fn stats_report_lines(args: StatsArgs) -> Result<Vec<String>> {
    let report = build_stats_report(args).await?;
    Ok(format_report_lines(&report))
}

async fn build_stats_report(args: StatsArgs) -> Result<Vec<ReportLine>> {
    ...
    check_video_sources(&document.metadata, markdown_dir, &mut report).await;
...
async fn check_video_sources(
    metadata: &VideoMetadata,
    markdown_dir: &Path,
    report: &mut Vec<ReportLine>,
) {
    let config = match crate::video::config::VideoConfig::load() {
        Ok(config) => config,
        Err(error) => {
            report.push(ReportLine::new(
                Level::Error,
                "video.stats.video_metadata",
                format!("Unable to load video config: {error}"),
            ));
            return;
        }
    };

    match resolve_video_sources(metadata, markdown_dir, &config).await {
        Ok(resolved) => {
