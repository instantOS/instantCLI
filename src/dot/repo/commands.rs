...
pub fn handle_repo_command(
    ...
    match command {
        ...
        RepoCommands::Lazygit { name } => open_repo_lazygit(config, db, name.as_deref()),
        RepoCommands::Shell { name } => open_repo_shell(config, db, name.as_deref()),
    }
}
...
fn open_repo_lazygit(config: &Config, db: &Database, name: Option<&str>) -> Result<()> {
...
fn open_repo_shell(config: &Config, db: &Database, name: Option<&str>) -> Result<()> {
    let repo_name = match name {
        Some(n) => n.to_string(),
