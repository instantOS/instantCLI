...
pub fn configure_mouse_sensitivity(ctx: &mut SettingsContext) -> Result<()> {
    let key = super::store::IntSettingKey::new("desktop.mouse.sensitivity", 50);
...
pub fn restore_natural_scroll(ctx: &mut SettingsContext) -> Result<()> {
    let key = super::store::BoolSettingKey::new("desktop.mouse.natural_scroll", false);
...
pub fn restore_swap_buttons(ctx: &mut SettingsContext) -> Result<()> {
    let key = super::store::BoolSettingKey::new("desktop.mouse.swap_buttons", false);
